...
export function InvitesPanel({ divvyId }: { divvyId: string }) {
  ...
  async function revoke(inviteId: string) {
    const res = await fetch(`/api/groups/${divvyId}/invites/${inviteId}/revoke`, {
      method: 'POST',
    });
    const data = await res.json();
  ...
  async function resendInvite(inviteId: string) {
    const res = await fetch(`/api/groups/${divvyId}/invites/${inviteId}/resend`, {
      method: 'POST',
    });
    const data = await res.json();
  ...
  return (
    <div className="border rounded p-4 space-y-4">
      ...
      <form
        ...
        className="grid gap-2 md:grid-cols-[1fr,140px,120px]"
      >
        <input
      ...
      {loading ? (
        ...
        <>
          ...
          <div className="space-y-2">
            ...
            {invites.length === 0 && (
              <p className="opacity-70">Nenhum convite ainda.</p>
            )}
          </div>
        </>
      )}
