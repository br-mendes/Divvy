...
export async function POST(_req: Request, ctx: { params: { divvyId: string; requestId: string } }) {
  ...
  const type = String((reqRow as any).type ?? '');
  const status = String((reqRow as any).status ?? '');
  if (status !== 'pending') {
  ...
  const patch: any = { status: 'approved', decided_by: user.id, decidedat: new Date().toISOString(), updatedat: new Date().toISOString() };
